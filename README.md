...
- Checks PBS job status via `ssh happiness qstat` every 5 minutes
...
1. Copy `.env.example` to `.env` and fill in your healthchecks.io API key:
   ```
...
3. For each job in `jobs.json`:
   ...
   - Queries `qstat -f` via SSH to Happiness
   - Evaluates job state:
